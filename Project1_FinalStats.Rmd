...
data_CH4$LC <- trimws(data_CH4$LC)
data_CO2$LC <- trimws(data_CO2$LC)

LC_dfCH4_new <- data_CH4 %>%
  select(-HM, -DEM) %>%       # remove HM and DEM
  filter(!LC %in% c("Nonliving", "Road", "NoData"))

LC_dfCO2_new <- data_CO2 %>%
  select(-HM, -DEM) %>%
  filter(!LC %in% c("Nonliving", "Road", "NoData"))
```

## Co-occurrence
This section was used to evaluate bin sizes for size similarity and determine the overlap between landscape factors. It was further aimed at helping us understand where cross affiliation occurs (even somewhat naturally) with gas fluxes and spatial heterogeneity.
```{r}
library(dplyr)
library(ggplot2)
...
table(data_CH4$DEM_bin)


# ----- PLANT HEIGHT BINS -----

data_CH4$HM_bin <- cut(data_CH4$HM,
                              breaks = c(0, 0.1, 0.5, 1, Inf),
                              labels = c("<0.1 m,", "0.1-0.5 m",
                                        "0.5-1.0 m",  ">1 m"),
                              right = FALSE)  
                          # Use right = FALSE to make intervals like [0,1)
...
table(data_CH4$HM_bin)

# Filter out NAs and remove unused classes from LC
data_filtered <- data_CH4 %>%
  filter(!is.na(DEM_bin), !is.na(HM_bin), !is.na(LC)) %>%
  filter(!LC %in% c("Nonliving", "Road", "NoData"))

# --- DEM vs HM ---
df_dem_hm <- data_filtered %>%
  group_by(DEM_bin, HM_bin) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(DEM_bin) %>%
  mutate(prop = count / sum(count)) %>%
  ungroup()

p1<-ggplot(df_dem_hm, aes(x = HM_bin, y = DEM_bin, fill = prop)) +
...
p2<-ggplot(df_dem_veg, aes(x = LC, y = DEM_bin, fill = prop)) +
  geom_tile(color = "white") +
...
p3<-ggplot(df_hm_veg, aes(x = LC, y = HM_bin, fill = prop)) +
  geom_tile(color = "white") +
...
#Export figures to designated folder
 setwd("C:/MontezumaData/2024_Data/EC_Data/Images/R_plots")
 
  #PNG output
ggsave(
  filename = "Cooccurrence.png",
  plot = combined_plot,
...
modes_CO2_night <- data_CO2 %>%
  filter(!is.na(AllCO2n), !is.na(HM_bin)) %>%
  group_by(HM_bin) %>%
  group_modify(~ find_density_peaks(.x$AllCO2n, bw_mult = 2.5))

print(n=30, modes_CO2_night)
```
...
shared_scale <- scale_color_manual(
  values = c("#E69F00", "#CC99FF","#66A61E", "black", "#2C7FB8"))
